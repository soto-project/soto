...
class IAMTests: XCTestCase {
    ...
    func testSetGetPolicy() {
        attempt {
            ...
            let deleteRequest = IAM.DeleteUserPolicyRequest(policyName: "testSimulatePolicy", userName: testData.userName)
            _ = try client.deleteUserPolicy(deleteRequest).wait()
        }
