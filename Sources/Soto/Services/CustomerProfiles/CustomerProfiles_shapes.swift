...
extension CustomerProfiles {
    ...
    public struct RecommendationMetadata: AWSEncodableShape {
        /// A list of metadata column names from your Items dataset to include in the recommendation response.
        ...
        public func validate(name: String) throws {
            try self.columns?.forEach {
                try validate($0, name: "columns[]", parent: name, max: 150)
    ...
    public struct Recommender: AWSEncodableShape {
        /// A list of filters to apply to the returned recommendations. Filters define criteria for including or excluding items from the recommendation results.
        public let filters: [RecommenderFilter]?
        /// The unique name of the recommender.
        public let name: String
        /// A list of promotional filters to apply to the recommendations. Promotional filters allow you to promote specific items within a configurable subset of recommendation results.
        public let promotionalFilters: [RecommenderPromotionalFilter]?

        @inlinable
        public init(filters: [RecommenderFilter]? = nil, name: String, promotionalFilters: [RecommenderPromotionalFilter]? = nil) {
            self.filters = filters
            self.name = name
            self.promotionalFilters = promotionalFilters
        }
        ...
        public func validate(name: String) throws {
            ...
            try self.validate(self.name, name: "name", parent: name, min: 1)
            try self.validate(self.name, name: "name", parent: name, pattern: "^[a-zA-Z0-9_-]+$")
            try self.promotionalFilters?.forEach {
                try $0.validate(name: "\(name).promotionalFilters[]")
            }
            try self.validate(self.promotionalFilters, name: "promotionalFilters", parent: name, max: 1)
        }

        private enum CodingKeys: String, CodingKey {
            case filters = "Filters"
            case name = "Name"
            case promotionalFilters = "PromotionalFilters"
        }
    }
    ...
    public struct SearchRecommendationsRequest: AWSEncodableShape {
        /// A list of item IDs to rank for the user. Use this when you want to re-rank a specific set of items rather than getting recommendations from the full item catalog. Required for personalized-ranking use cases.
        public let candidateIds: [String]?
        /// The contextual metadata used to provide dynamic runtime information to tailor recommendations.
        public let context: [String: String]?
        /// Runtime diversity configuration for this request. Enables diversity-aware recommendations and optionally supplies values for placeholder-based diversity caps configured on the recommender.
        public let diversity: RecommendationDiversityConfig?
        /// The unique name of the domain.
        public let domainName: String
        ...
        public let metadata: RecommendationMetadata?
        /// The recommender used to generate the recommendations.
        public let recommender: Recommender

        @inlinable
        public init(candidateIds: [String]? = nil, context: [String: String]? = nil, diversity: RecommendationDiversityConfig? = nil, domainName: String, keyName: String, keyValues: [String], maxRecommendations: Int? = nil, metadata: RecommendationMetadata? = nil, recommender: Recommender) {
            self.candidateIds = candidateIds
            self.context = context
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            try container.encodeIfPresent(self.candidateIds, forKey: .candidateIds)
            try container.encodeIfPresent(self.context, forKey: .context)
            try container.encodeIfPresent(self.diversity, forKey: .diversity)
            request.encodePath(self.domainName, key: "DomainName")
            try container.encode(self.keyName, forKey: .keyName)
            try container.encode(self.keyValues, forKey: .keyValues)
            try container.encodeIfPresent(self.maxRecommendations, forKey: .maxRecommendations)
            try container.encodeIfPresent(self.metadata, forKey: .metadata)
            try container.encode(self.recommender, forKey: .recommender)
        }
        ...
        public func validate(name: String) throws {
            ...
            try self.diversity?.validate(name: "\(name).diversity")
            try self.validate(self.domainName, name: "domainName", parent: name, max: 64)
            try self.validate(self.domainName, name: "domainName", parent: name, min: 1)
            try self.validate(self.domainName, name: "domainName", parent: name, pattern: "^[a-zA-Z0-9_-]+$")
            try self.validate(self.keyName, name: "keyName", parent: name, max: 64)
            try self.validate(self.keyName, name: "keyName", parent: name, min: 1)
            try self.validate(self.keyName, name: "keyName", parent: name, pattern: "^[a-zA-Z0-9_-]+$")
            try self.keyValues.forEach {
                try validate($0, name: "keyValues[]", parent: name, max: 255)
                try validate($0, name: "keyValues[]", parent: name, min: 1)
            }
            try self.validate(self.keyValues, name: "keyValues", parent: name, max: 10)
            try self.validate(self.keyValues, name: "keyValues", parent: name, min: 1)
            try self.validate(self.maxRecommendations, name: "maxRecommendations", parent: name, max: 500)
            try self.validate(self.maxRecommendations, name: "maxRecommendations", parent: name, min: 1)
            try self.metadata?.validate(name: "\(name).metadata")
            try self.recommender.validate(name: "\(name).recommender")
        }
        ...
        private enum CodingKeys: String, CodingKey {
            ...
            case maxRecommendations = "MaxRecommendations"
            case metadata = "Metadata"
            case recommender = "Recommender"
        }
    }

    public struct SearchRecommendationsResponse: AWSDecodableShape {
        /// The unique identifier of the profile for which to retrieve recommendations.
        public let profileId: String?
        /// List of recommendations generated by the recommender.
        public let recommendations: [Recommendation]?
        ...
        @inlinable
        public init(profileId: String? = nil, recommendations: [Recommendation]? = nil) {
            self.profileId = profileId
            self.recommendations = recommendations
        }
