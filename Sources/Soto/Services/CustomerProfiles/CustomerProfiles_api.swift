...
public struct CustomerProfiles: AWSService {
    ...
    @Sendable
    @inlinable
    public func searchRecommendations(_ input: SearchRecommendationsRequest, logger: Logger = AWSClient.loggingDisabled) async throws -> SearchRecommendationsResponse {
        try await self.client.execute(
            operation: "SearchRecommendations", 
            path: "/domains/{DomainName}/recommendations", 
            httpMethod: .POST, 
            serviceConfig: self.config, 
            input: input, 
            logger: logger
    ...
    ///   - candidateIds: A list of item IDs to rank for the user. Use this when you want to re-rank a specific set of items rather than getting recommendations from the full item catalog. Required for personalized-ranking use cases.
    ///   - context: The contextual metadata used to provide dynamic runtime information to tailor recommendations.
    ///   - diversity: Runtime diversity configuration for this request. Enables diversity-aware recommendations and optionally supplies values for placeholder-based diversity caps configured on the recommender.
    ///   - domainName: The unique name of the domain.
    ///   - keyName: A searchable identifier of a customer profile. You can use a predefined key, such as _profileId, _phone, or _email, or a custom-defined key.
    ///   - keyValues: A list of key values. Provide one value for each field of the search key.
    ///   - maxRecommendations: The maximum number of recommendations to return. The default value is 5.
    ///   - metadata: Configuration for metadata to include in recommendation responses.
    ///   - recommender: The recommender used to generate the recommendations.
    ///   - logger: Logger use during operation
    @inlinable
    public func searchRecommendations(
        candidateIds: [String]? = nil,
        context: [String: String]? = nil,
        diversity: RecommendationDiversityConfig? = nil,
        domainName: String,
        keyName: String,
        keyValues: [String],
        ...
        logger: Logger = AWSClient.loggingDisabled        
    ) async throws -> SearchRecommendationsResponse {
        let input = SearchRecommendationsRequest(
            candidateIds: candidateIds, 
            context: context, 
            diversity: diversity, 
            domainName: domainName, 
        ...
        return try await self.searchRecommendations(input, logger: logger)
    }
