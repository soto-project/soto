...
extension ElastiCache {
    ...
    public func testMigration(_ input: TestMigrationMessage, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> TestMigrationResponse {
        return try await self.client.execute(operation: "TestMigration", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
}
