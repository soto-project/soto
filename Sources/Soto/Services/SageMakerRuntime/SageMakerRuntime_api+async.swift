...
extension SageMakerRuntime {
    ...
    public func invokeEndpointWithResponseStream(_ input: InvokeEndpointWithResponseStreamInput, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> InvokeEndpointWithResponseStreamOutput {
        return try await self.client.execute(operation: "InvokeEndpointWithResponseStream", path: "/endpoints/{EndpointName}/invocations-response-stream", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
}
