...
public struct SageMakerRuntime: AWSService {
    ...
    public func invokeEndpointWithResponseStream(_ input: InvokeEndpointWithResponseStreamInput, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<InvokeEndpointWithResponseStreamOutput> {
        return self.client.execute(operation: "InvokeEndpointWithResponseStream", path: "/endpoints/{EndpointName}/invocations-response-stream", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
}
