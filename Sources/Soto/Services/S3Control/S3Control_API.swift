...
public struct S3Control: AWSService {
    ...
    public func describeMultiRegionAccessPointOperation(_ input: DescribeMultiRegionAccessPointOperationRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeMultiRegionAccessPointOperationResult> {
        return self.client.execute(operation: "DescribeMultiRegionAccessPointOperation", path: "/v20180820/async-requests/mrap/{request_token+}", httpMethod: .GET, serviceConfig: self.config, input: input, hostPrefix: "{AccountId}.", logger: logger, on: eventLoop)
    }
