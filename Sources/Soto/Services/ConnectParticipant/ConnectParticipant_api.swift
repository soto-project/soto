...
public struct ConnectParticipant: AWSService {
    ...
    public func describeView(_ input: DescribeViewRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DescribeViewResponse> {
        return self.client.execute(operation: "DescribeView", path: "/participant/views/{ViewToken}", httpMethod: .GET, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
