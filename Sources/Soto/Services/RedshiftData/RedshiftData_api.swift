...
public struct RedshiftData: AWSService {
    ...
    ///   - waitTimeSeconds: The number of seconds to wait for the SQL statement to complete execution before returning the description. The maximum value is 30 seconds.
    ///   - logger: Logger use during operation
    @inlinable
    ...
    ///   - waitTimeSeconds: The number of seconds to wait for the SQL statement to complete execution before returning the response. If the SQL statement does not complete within the specified time, the response returns the current status. The maximum value is 30 seconds.
    ...
    ///   - nextToken: A value that indicates the starting point for the next set of response records in a subsequent request. If a value is returned in a response, you can retrieve the next set of records by providing this returned NextToken value in the next NextToken parameter and retrying the command. If the NextToken field is empty, all response records have been retrieved for the request.
    ///   - waitTimeSeconds: The number of seconds to wait for the SQL statement to complete execution before returning the result. The maximum value is 30 seconds.
    ///   - logger: Logger use during operation
    @inlinable
    ...
    ///   - status: The status of the sessions to list. If no status is specified, sessions with a status of AVAILABLE or BUSY are returned. Status values are defined as follows:   AVAILABLE – The session is open and ready to run a SQL statement.   BUSY – The session is currently running a SQL statement.   CLOSED – The session is closed and can no longer run SQL statements.
    ...
    public func listSessions(
        ...
        roleLevel: Bool? = nil,
        sessionId: String? = nil,
        status: SessionStatusString? = nil,
        workgroupName: String? = nil,
        logger: Logger = AWSClient.loggingDisabled        
...
extension RedshiftData.ListSessionsRequest: AWSPaginateToken {
    @inlinable
    public func usingPaginationToken(_ token: String) -> RedshiftData.ListSessionsRequest {
        return .init(
            clusterIdentifier: self.clusterIdentifier,
            database: self.database,
            maxResults: self.maxResults,
