...
public struct Translate: AWSService {
    ...
    public func translateDocument(_ input: TranslateDocumentRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<TranslateDocumentResponse> {
        return self.client.execute(operation: "TranslateDocument", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
