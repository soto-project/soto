...
extension GuardDuty {
    ...
    public func startMalwareScan(_ input: StartMalwareScanRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> StartMalwareScanResponse {
        return try await self.client.execute(operation: "StartMalwareScan", path: "/malware-scan/start", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
