...
public struct PaymentCryptographyData: AWSService {
    ...
    ///
    /// Parameters:
    ///   - kekValidationType: Defines whether to generate a KEK validation request or KEK validation response for node-to-node initialization.
    ...
    @Sendable
    @inlinable
    public func generateAuthRequestCryptogram(_ input: GenerateAuthRequestCryptogramInput, logger: Logger = AWSClient.loggingDisabled) async throws -> GenerateAuthRequestCryptogramOutput {
        try await self.client.execute(
            operation: "GenerateAuthRequestCryptogram", 
            path: "/cryptogram/generate", 
            httpMethod: .POST, 
            serviceConfig: self.config, 
            input: input, 
            logger: logger
    ...
    ///
    /// Parameters:
    ///   - keyIdentifier: The keyARN of the IMK-AC (TR31_E0_EMV_MKEY_APP_CRYPTOGRAMS) that Amazon Web Services Payment Cryptography uses to generate the ARQC.
    ///   - majorKeyDerivationMode: The method to use when deriving the major encryption key for ARQC generation within Amazon Web Services Payment Cryptography.
    ///   - sessionKeyDerivationAttributes: The attributes and values to use for deriving a session key for ARQC generation within Amazon Web Services Payment Cryptography.
    ///   - transactionData: The transaction data that Amazon Web Services Payment Cryptography uses for ARQC generation. The same transaction data is used for ARQC verification by the issuer using VerifyAuthRequestCryptogram.
    ///   - logger: Logger use during operation
    @inlinable
    public func generateAuthRequestCryptogram(
        keyIdentifier: String,
        majorKeyDerivationMode: MajorKeyDerivationMode,
        sessionKeyDerivationAttributes: SessionKeyDerivation,
        transactionData: String,
        logger: Logger = AWSClient.loggingDisabled        
    ) async throws -> GenerateAuthRequestCryptogramOutput {
        let input = GenerateAuthRequestCryptogramInput(
            keyIdentifier: keyIdentifier, 
            majorKeyDerivationMode: majorKeyDerivationMode, 
            sessionKeyDerivationAttributes: sessionKeyDerivationAttributes, 
            transactionData: transactionData
        )
        return try await self.generateAuthRequestCryptogram(input, logger: logger)
    }
    ...
    public func generatePinData(
        ...
        primaryAccountNumber: String? = nil,
        logger: Logger = AWSClient.loggingDisabled        
    ...
    public func translateKeyMaterial(_ input: TranslateKeyMaterialInput, logger: Logger = AWSClient.loggingDisabled) async throws -> TranslateKeyMaterialOutput {
        try await self.client.execute(
            operation: "TranslateKeyMaterial", 
    ...
    ///   - logger: Logger use during operation
    @inlinable
    public func translateKeyMaterial(
        ...
        logger: Logger = AWSClient.loggingDisabled        
    ) async throws -> TranslateKeyMaterialOutput {
        let input = TranslateKeyMaterialInput(
            incomingKeyMaterial: incomingKeyMaterial, 
            keyCheckValueAlgorithm: keyCheckValueAlgorithm, 
            outgoingKeyMaterial: outgoingKeyMaterial
        )
        return try await self.translateKeyMaterial(input, logger: logger)
    }
