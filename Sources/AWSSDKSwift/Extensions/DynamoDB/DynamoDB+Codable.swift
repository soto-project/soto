...
extension DynamoDB {
    ...
    public func putItem<T: Encodable>(_ input: PutItemCodableInput<T>) -> EventLoopFuture<PutItemOutput> {
        do {
            ...
            return self.putItem(request)
        } catch {
            let eventLoop = context.eventLoop ?? client.eventLoopGroup.next()
    ...
    public func getItem<T: Decodable>(_ input: GetItemInput, type: T.Type) -> EventLoopFuture<GetItemCodableOutput<T>> {
        return self.getItem(input)
    ...
    public func updateItem<T: Encodable>(_ input: UpdateItemCodableInput<T>) -> EventLoopFuture<UpdateItemOutput> {
        do {
            ...
            return self.updateItem(request)
        } catch {
            let eventLoop = self.context.eventLoop ?? client.eventLoopGroup.next()
            return eventLoop.makeFailedFuture(error)
        }
    }
    ...
    public func query<T: Decodable>(_ input: QueryInput, type: T.Type) -> EventLoopFuture<QueryCodableOutput<T>> {
    ...
    public func scan<T: Decodable>(_ input: ScanInput, type: T.Type) -> EventLoopFuture<ScanCodableOutput<T>> {
    ...
    public func queryPaginator<T: Decodable>(
        ...
        return client.paginate(input: input, command: self.query, tokenKey: \QueryOutput.lastEvaluatedKey, context: self.context) { (response, eventLoop) -> EventLoopFuture<Bool> in
            do {
                let items = try response.items.map { try $0.map { try DynamoDBDecoder().decode(T.self, from: $0) } }
    ...
    public func scanPaginator<T: Decodable>(
        ...
        return client.paginate(input: input, command: self.scan, tokenKey: \ScanOutput.lastEvaluatedKey, context: self.context) { (response, eventLoop) -> EventLoopFuture<Bool> in
            do {
                let items = try response.items.map { try $0.map { try DynamoDBDecoder().decode(T.self, from: $0) } }
