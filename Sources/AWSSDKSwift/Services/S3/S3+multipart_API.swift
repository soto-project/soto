...
public extension S3 {
        ...
        let result = headObject(headRequest)
            ...
            .then { object -> Future<Int64> in
                do {
                    ...
                    // download file
                    return multipartDownloadPart(fileSize: contentLength, offset: 0, prevPartSave: eventLoop.newSucceededFuture(result: contentLength))
                } catch {
                    return eventLoop.newFailedFuture(error: error)
                }
        }
    ...
    func multipartUpload(_ input: CreateMultipartUploadRequest, on eventLoop: EventLoop, inputStream: @escaping () throws -> Data?) -> Future<CompleteMultipartUploadOutput> {
        ...
        func multipartUploadPart(partNumber: Int, uploadId: String, body: Data? = nil) -> Future<[S3.CompletedPart]> {
            ...
            if let body = body {
                ...
                uploadResult = self.uploadPart(request).hopTo(eventLoop: eventLoop).map { output -> [S3.CompletedPart] in
                    let part = S3.CompletedPart(eTag: output.eTag, partNumber: partNumber)
                    completedParts.append(part)
                    return completedParts
                }
            } else {
                uploadResult = eventLoop.newSucceededFuture(result: [])
            }
            ...
            let result = eventLoop.submit {
                ...
                .then { (data, parts) -> Future<[S3.CompletedPart]> in
                    guard let data = data, data.count > 0 else { return eventLoop.newSucceededFuture(result: parts)}
        ...
        let result = createMultipartUpload(input).then { upload -> Future<CompleteMultipartUploadOutput> in
            guard let uploadId = upload.uploadId else { return eventLoop.newFailedFuture(error: S3ErrorType.multipart.noUploadId) }
    ...
    func multipartUpload(_ input: CreateMultipartUploadRequest, partSize: Int = 5*1024*1024, filename: String, on eventLoop: EventLoop = AWSClient.eventGroup.next(), progress: @escaping (Double) throws->() = { _ in }) -> Future<CompleteMultipartUploadOutput> {
        ...
        }.then { (fileHandle) -> Future<CompleteMultipartUploadOutput> in
