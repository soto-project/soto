...
extension KinesisVideoArchivedMedia {
    ...
    public func listFragmentsPaginator(_ input: ListFragmentsInput, onPage: @escaping ([Fragment], EventLoop)->EventLoopFuture<Bool>) -> EventLoopFuture<Void> {
        return client.paginate(input: input, command: listFragments, resultKey: \ListFragmentsOutput.fragments, tokenKey: \ListFragmentsOutput.nextToken, onPage: onPage)
    }
...
extension KinesisVideoArchivedMedia.ListFragmentsInput: AWSPaginateStringToken {
    public init(_ original: KinesisVideoArchivedMedia.ListFragmentsInput, token: String) {
        self.init(
            ...
            nextToken: token, 
            streamName: original.streamName
        )
    }
}
