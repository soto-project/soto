...
extension SimpleDB {
    ...
    public func listDomainsPaginator(_ input: ListDomainsRequest) -> EventLoopFuture<[String]> {
        return client.paginate(input: input, command: listDomains, resultKey: \ListDomainsResult.domainNames, tokenKey: \ListDomainsResult.nextToken)
    }
    ...
    public func selectPaginator(_ input: SelectRequest) -> EventLoopFuture<[Item]> {
        return client.paginate(input: input, command: select, resultKey: \SelectResult.items, tokenKey: \SelectResult.nextToken)
    }
