...
public struct ForecastQueryService: AWSService {
    ...
    /// return new `ForecastQueryService` with new timeout value
    ...
    /// return new `ForecastQueryService` logging to specified Logger
    ...
    public func queryForecast(_ input: QueryForecastRequest, on eventLoop: EventLoop? = nil) -> EventLoopFuture<QueryForecastResponse> {
        return client.execute(operation: "QueryForecast", path: "/", httpMethod: .POST, serviceContext: context, input: input, on: eventLoop)
    }
}
